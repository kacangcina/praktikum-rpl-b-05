...
class RecipeFlowTest extends TestCase
{
    ...
    public function test_search_results_are_ordered_by_keyword_relevance(): void
    {
        $ingredientMatch = $this->createRecipe();
        $ingredientMatch->update(['title' => 'Sup Rumahan']);
        $ingredientMatch->ingredients()->create([
            'ingredient_name' => 'Soto',
            'quantity' => '1 bungkus',
        ]);

        $partialTitle = $this->createRecipe();
        $partialTitle->update(['title' => 'Resep Soto Betawi']);

        $startsWithTitle = $this->createRecipe();
        $startsWithTitle->update(['title' => 'Soto Ayam']);

        $exactTitle = $this->createRecipe();
        $exactTitle->update(['title' => 'Soto']);

        $this->getJson('/api/recipes?q=Soto')
            ->assertOk()
            ->assertJsonPath('recipes.0.title', 'Soto')
            ->assertJsonPath('recipes.1.title', 'Soto Ayam')
            ->assertJsonPath('recipes.2.title', 'Resep Soto Betawi')
            ->assertJsonPath('recipes.3.title', 'Sup Rumahan');
    }

    public function test_empty_search_result_recommends_at_most_three_highest_rated_recipes(): void
    {
        $ratings = [
            'Rating Empat' => [4],
            'Rating Lima Sedikit' => [5],
            'Rating Lima Banyak' => [5, 5],
            'Rating Tiga' => [3],
        ];

        foreach ($ratings as $title => $values) {
            $recipe = $this->createRecipe();
            ...
            foreach ($values as $rating) {
                $recipe->reviews()->create([
                    ...
                    'rating' => $rating,
                    'comment' => 'Ulasan rekomendasi.',
                ]);
            }
        }

        $this->getJson('/api/recipes?q=BahanYangTidakAda')
            ->assertOk()
            ->assertJsonCount(0, 'recipes')
            ->assertJsonCount(3, 'recommendations')
            ->assertJsonPath('recommendations.0.title', 'Rating Lima Banyak')
            ->assertJsonPath('recommendations.1.title', 'Rating Lima Sedikit')
            ->assertJsonPath('recommendations.2.title', 'Rating Empat')
            ->assertJsonPath('recommendations.0.average_rating', 5)
            ->assertJsonPath('recommendations.0.reviews_count', 2);
    }

    public function test_search_does_not_return_recommendations_when_a_match_exists(): void
    {
        $this->createRecipe();
    ...
    public function test_recipe_cards_include_average_rating_and_review_count(): void
    {
        $recipe = $this->createRecipe();

        foreach ([4, 5, 5] as $rating) {
            $recipe->reviews()->create([
                'user_id' => User::factory()->create()->id,
                'rating' => $rating,
                'comment' => 'Ulasan untuk badge rating.',
            ]);
        }

        $response = $this->getJson('/api/recipes')->assertOk();
        $card = collect($response->json('recipes'))->firstWhere('id', $recipe->id);

        $this->assertSame(4.7, $card['average_rating']);
        $this->assertSame(3, $card['reviews_count']);
    }
