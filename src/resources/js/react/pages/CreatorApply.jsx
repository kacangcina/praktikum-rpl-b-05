        ...
        <div className="mx-auto max-w-4xl">
            <div className="flex items-center justify-between border-b border-stone-400 pb-5">
                ...
                <div className="flex items-center gap-6">
                    <span className="rounded-3xl border border-red-400 bg-red-200 px-5 py-3 capitalize">{latest?.status || 'Belum diajukan'}</span>
                    <Link to="/profile" aria-label="Kembali ke profil"><X size={40} /></Link>
                </div>
            </div>
            ...
            {!blocked && <form onSubmit={async (event) => {
                ...
                <Field label="URL portofolio"><input aria-label="URL portofolio" type="url" name="portfolio_url" placeholder="https://..." /></Field>
                <Field label="Deskripsi diri sebagai chef"><textarea aria-label="Deskripsi diri sebagai chef" name="notes" rows="4" placeholder="Ceritakan pengalaman memasakmu..." required /></Field>
                <FileUploadField
                    name="document"
                    accept=".pdf,image/jpeg,image/png"
                    required
                    label="Dokumen KTP"
                    hint="PDF, JPG, atau PNG. Preview tampil sebelum pengajuan dikirim."
                    kind="document"
                />
                <UploadBox title="Portofolio memasak" subtitle="Gunakan URL portofolio di atas" icon={<FileText />} />
                <div className="flex justify-end gap-4 border-t border-stone-400 pt-7"><Link to="/profile" className="wire-button bg-stone-200">Batal</Link><button type="submit" className="wire-button wire-button-primary">Kirim pengajuan</button></div>
            </form>}
        </div>
    );
}
