import { Bell, Bookmark, Home, LogOut, Menu, MessageCircleQuestion, Search, ShieldCheck, UserRound } from 'lucide-react';
        ...
        <header className="sticky top-0 z-50 border-b border-stone-300 bg-white/95 backdrop-blur">
            <div className="page-container relative flex min-h-20 items-center gap-4 sm:gap-6">
                ...
                <nav className="hidden items-center gap-5 text-sm font-semibold lg:flex">
                    ...
                    {user && <NavItem to="/consultation" icon={<MessageCircleQuestion size={18} />}>Konsultasi AI</NavItem>}
                    {user?.is_admin && <NavItem to="/admin" icon={<ShieldCheck size={18} />}>Admin</NavItem>}
                </nav>
                ...
                <form onSubmit={submitSearch} className="ml-auto hidden max-w-sm flex-1 md:flex">
                    <div className="flex w-full items-center rounded-full bg-stone-200 px-4">
                        ...
                        <input name="q" className="w-full bg-transparent px-3 py-2.5 text-sm outline-none" placeholder="Cari resep" aria-label="Cari resep" />
                    </div>
                    <button type="submit" aria-label="Cari resep" className="sr-only">Cari</button>
                </form>

                {user ? (
                    <div className="hidden items-center gap-2 border-l border-stone-400 pl-5 md:flex">
                        <Link to={`/profile/${user.id}`} className="relative rounded-xl p-2 hover:bg-stone-100" title="Pusat aktivitas" aria-label="Pusat aktivitas">
                            <Bell size={20} />
                            {user.unread_notifications_count > 0 && <span className="absolute -top-1 -right-1 grid min-w-5 place-items-center rounded-full bg-orange-500 px-1 text-[10px] font-black text-white">{Math.min(user.unread_notifications_count, 99)}</span>}
                        </Link>
                ...
                <button type="button" onClick={() => setOpen(!open)} className="absolute right-0 rounded-xl p-2 hover:bg-stone-100 md:hidden" aria-label="Menu"><Menu /></button>
                {open && (
                    <div className="absolute top-[calc(100%+1px)] right-0 left-0 z-50 border border-stone-300 bg-white p-4 shadow-lg md:hidden">
                        <div className="grid gap-2 font-semibold">
                            ...
                            {user && <Link onClick={() => setOpen(false)} to="/consultation" className="rounded-xl px-3 py-2 hover:bg-stone-100">Konsultasi AI</Link>}
                            {user?.is_admin && <Link onClick={() => setOpen(false)} to="/admin" className="rounded-xl px-3 py-2 hover:bg-stone-100">Admin</Link>}
