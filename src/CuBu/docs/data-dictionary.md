...
| recipe_id | INT | FK → recipes.id, UNIQUE, NOT NULL | ID resep tempat video ditampilkan |
...
| portfolio_url | TEXT | NULL | Tautan portofolio publik pemohon |
| notes | TEXT | NOT NULL | Pengalaman memasak dan alasan mengajukan creator |
...
| reviewed_by | INT | FK → users.id, NULL | ID admin yang meninjau pengajuan |
